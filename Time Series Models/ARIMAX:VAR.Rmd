...
# Ensure that 'top10_gross' and 'closing_price' are in the correct numeric format
df$top10_gross <- as.numeric(df$top10_gross)
df$closing_price <- as.numeric(df$closing_price)

# Convert 'top10_gross' to a time series object
# The frequency should reflect the periodicity of your data (e.g., weekly)
top10_gross_ts <- ts(df$top10_gross, frequency = 52)

# Use 'auto.arima' to fit the ARIMAX model
# 'closing_price' is the exogenous variable
arimax_model <- auto.arima(top10_gross_ts, xreg = df$closing_price)
...
arimax_model <- Arima(top10_gross_ts, order = c(1,1,4), xreg = df$closing_price, include.constant = FALSE)

# Manual model fitting
arimax_model_411 <- Arima(top10_gross_ts, order = c(4,1,1), seasonal = list(order = c(1,1,1), period = 52), xreg = df$closing_price)
...
# Assuming top10_gross_ts is your time series and df$closing_price is your exogenous variable

# Fit the ARIMA(4,1,1) model with seasonal(1,1,1) and exogenous variable
arimax_model_411 <- Arima(top10_gross_ts, order = c(4, 1, 1), seasonal = list(order = c(1, 1, 1), period = 52), xreg = df$closing_price)

# Number of periods you want to forecast
forecast_horizon <- 100  # You can adjust this number as needed

# Generate forecasts
arimax_forecast <- forecast(arimax_model_411, h = forecast_horizon, xreg = tail(df$closing_price, forecast_horizon))
...
plot(top10_gross_ts, main = "ARIMA(4,1,1) with Seasonal(1,1,1) Forecast", xlab = "Time", ylab = "Value", col = "blue")

# Add the forecasted values to the plot
lines(arimax_forecast$mean, col = "red", lty = 2)  # Mean forecast

# Add a legend
legend("topright", legend = c("Actual", "Forecast"), col = c("blue", "red"), lty = c(1, 2, 2))
...
# Function to perform sliding window cross-validation for ARIMA model with exogenous variable
sliding_window_cv_arima <- function(order, xreg = NULL) {
  rmse_values <- c()
  ts_length <- length(top10_gross_ts)
  fold_size <- 52  # Sliding window size
  
  for (i in 1:(ts_length - fold_size - num_folds + 2)) {
    start_idx <- i
    end_idx <- start_idx + num_folds * fold_size - 1
    
    train_data <- window(top10_gross_ts, start = start_idx, end = end_idx - fold_size)
    test_data <- window(top10_gross_ts, start = end_idx - fold_size + 1, end = end_idx)
    
    arima_model <- Arima(train_data, order = order, xreg = window(df$closing_price, start = start_idx, end = end_idx - 1), include.constant = FALSE)
    forecast_values <- forecast(arima_model, h = length(test_data), xreg = window(df$closing_price, start = end_idx, end = end_idx + num_folds - 1))
    
    rmse_values <- c(rmse_values, rmse(test_data, forecast_values$mean))
  }
  
  return(rmse_values)
}

# Sliding window cross-validation for ARIMA(1,1,4) with exogenous variable
cv_rmse_values <- sliding_window_cv_arima(c(1, 1, 4), xreg = df$closing_price)

# Calculate and print the average RMSE
average_rmse <- mean(cv_rmse_values)
